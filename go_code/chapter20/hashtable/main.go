...
//方法待定
func (this *Emp) ShowMs() {
	fmt.Printf("链表%d  找到该雇员 %id\n", this.Id%7, this.Id)
}

// 定义EmpLink
// 我们这里EmpLink  不带表头，及第一个节点就存放雇员
type EmpLink struct {
	Head *Emp
}
...
func (this *EmpLink) Insert(emp *Emp) {
	...
	// 如果当前的 EmpLink， 就是一空链表
	if cur == nil {
		this.Head = emp //完成
		return
	}
	// 如果不是一个空链表，给emp找到对应的位置 并插入
	//思路是 让cur 和emp 比较 ， 然后让pre 保持在cur前面
	for {
		if cur != nil {
			// 进行比较
			if cur.Id > emp.Id {
