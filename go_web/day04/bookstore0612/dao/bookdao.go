...
func GetBooks() ([]*model.Book, error) {
	//写sql语句
	sqlStr := "select id,title,author,price,sales,stock,img_path from book"
	//执行
...
func AddBook(b *model.Book) error {
	//写sql语句
	slqStr := "insert into book (title,author,price,sales,stock,img_path) values(?,?,?,?,?,?)"
	//执行
...
func GetPageBooks(pageNo string) (*model.Page, error) {
	...
	*/
	//获取当前页中的图书
	sqlStr2 := "select id,title,author,price,sales,stock,img_path from book limit ?,?"
	//执行
...
func GetPageBooksByPrice(pageNo string, minPrice string, maxPrice string) (*model.Page, error) {
	...
	*/
	//获取当前页中的图书
	sqlStr2 := "select id,title,author,price,sales,stock,img_path from book where price between ? and ? limit ?,?"
	//执行
